...
## 🚀 インストール方法
.dmg ファイルをダウンロードして開き、アプリを /Applications フォルダに移動します。
初回起動時に「開発元を確認できないため〜」と表示される場合は、以下の手順を試してください。

### A. 右クリックで開く（推奨）
アプリを右クリックして「開く」を選択し、ダイアログで再度「開く」をクリックしてください。

### B. ターミナルで実行（上記で開けない場合）
以下のコマンドをコピーして実行してください：
`sudo xattr -rd com.apple.quarantine /Applications/TaxBuddy.app`
